  ...
  this.insert = data => {
    return new Promise(async (resolve, reject) => {
      ...
      instructors.forEach(instructor => {
        ...
        UserCourse.save().catch(err => {
          console.error("Error save UserCourse " + instructor + " : " + err);
        });
      });
    ...
    return new Promise((resolve, reject) => {
      UserCourseSchema.find({
        ...
        userCourses.forEach(userCourse => {
          CourseSchema.findById(userCourse.get('courseId')).exec().then(course => {
            courses.push(course)
          })
        });
        if (courses.length == 0) {
